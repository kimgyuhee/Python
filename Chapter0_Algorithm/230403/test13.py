...
문자열 배열 targets가 주어질 때, 각 문자열을 작성하기 위해 키를 최소 몇 번씩 눌러야 하는지 
...
def solution(keymap, targets):
    ...
    def minClick(keymap, target) :
        sum = 0
        for t in target : # ABCD
            print(f"{t} 확인중입니다.!!!")
            valueList = []
            count = 0
            for key in keymap :
                if t in key :
                    print(f"{t}는 {key}에 존재한다.\n")
                    valueList.append(key.find(t)+1)
                    print(f"{t}의 현재 최소값은 {min}입니다.")
                else :
                    continue
