...
def solution(str1, str2):
    ...
    for i in range(len(str1)-1) :
        value = str1[i]+str1[i+1]
        if value.isalpha() :
            two_words1.append(value)

    for i in range(len(str2)-1) :
        value = str2[i]+str2[i+1]
        if value.isalpha() :
            two_words2.append(value)
    ...
    if len(set1) == 0 and len(set2) == 0 :
        ...
        value_min = min(len(two_words1), len(two_words2))
        value_max = max(len(two_words1), len(two_words2))
        answer = value_min/value_max
    else :
        print(set1 & set2)
        ...
        answer = len(inter)/len(un)

    return int(answer * 65536)

print(solution("FRANCE", "french"))
print(solution("E=M*C^2", "e=m*c^2"))
print(solution("2", "e=m*c^2"))
...
def solution(str1, str2):
    ...
    if len(set1) == 0 and len(set2) == 0 :
        ...
        print("CASE3 >>> ", end=" ")
        # 두 집합 중 길이가 작은 집합 구하기
        if len(two_words1) <= len(two_words2) :
            min_list = two_words1.copy()
            max_list = two_words2.copy()
        else :
            min_list = two_words2.copy()
            max_list = two_words1.copy()
        ...
        for value in min_list :
            if value in max_list :
                inter.append(value)
                max_list.remove(value)

        union = two_words1 + two_words2
        # 합집합 구하기
        for value in inter :
            union.remove(value)
